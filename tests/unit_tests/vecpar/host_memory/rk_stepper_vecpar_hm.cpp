...
*/
TEST(rk_stepper_algo_vecpar, rk_stepper_hm_timed) {
    ...
    vecmem::vector<free_track_parameters> tracks_host(&host_mr);
    vecmem::vector<free_track_parameters> tracks_device(&host_mr);
    vecmem::vector<free_track_parameters> tracks_benchmark(&host_mr);
...
#pragma omp parallel for
    ...
    start_time = std::chrono::high_resolution_clock::now();
    rk_stepper_algorithm rk_stepper_algo;
    vecpar::parallel_map(rk_stepper_algo, host_mr, getConfig(), tracks_device, B);
    end_time = std::chrono::high_resolution_clock::now();
    ...
    detray::Logger::logTime(detray::Logger::buildFilename("omp_cpu_hm"), time_cpu.count());
    detray::Logger::logTime(detray::Logger::buildFilename("vecpar_cpu_hm"), time_par.count());
#else
    printf("Built for GPU\n");
    detray::Logger::logTime(detray::Logger::buildFilename("seq_cpu_hm"), time_cpu.count());
    detray::Logger::logTime(detray::Logger::buildFilename("vecpar_gpu_hm"), time_par.count());
#endif
